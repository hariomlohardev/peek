...
See `peek/CONTRIBUTING.md`. PRs welcome — add tests per TDD. Branch `themes-10` is latest (from `design-anthropic`), local commits only, no push per workflow. Run `ruff`, `pytest -q` (72 passed) before PR.
