...
SCENES = [
    (
        ...
        [
            ...
            "  ─────────────────────────",
            "  5 seconds → map any repo (polyglot)",
            "  where to start • what talks to what",
            "",
            "  ▶  peek --help  →  peek . --no-tui  →  peek graph --format svg",
        ],
        None,
    ),
    (
        ...
        [
            ...
            "             find    keyword/semantic    config  theme persist",
            "             graph   DOT/SVG/HTML        mcp     MCP server",
    ...
    (
        ...
        "$ peek . --no-tui",
        [
            "  peek  v0.3.0  —  ./peek  (0.12s)",
            "  ┌ Languages ──────────────────┐ ┌ Tech Stack ────────┐",
            "  │ Python  ██████████████  68%  │ │ Primary: python  │",
            "  │ JavaScript ░░░░░░░░░░░  12%  │ │ Frameworks: textual, rich",
            "  └─────────────────────────────┘ └────────────────────┘",
            "  ┌ Start Here ⭐ (ranked) ─────────────────────────┐",
    ...
    (
        'peek find "auth token" --semantic',
        '$ peek find "auth token" --semantic',
        [
            "  peek find  v0.3.0  —  query: \"auth token\"  (semantic, 0.04s)",
            "  ┌ Matches (3 files, BM25 + fastembed) ───────────┐",
            "  │ 1  peek/peek/auth.py        12.4  semantic    │",
            "  │ 2  peek/peek/middleware.py   4.1  semantic • ranked 9.2 │",
            "  │ 3  tests/test_auth.py        3.2  semantic    │",
            "  └────────────────────────────────────────────────┘",
            "  Preview: auth.py:42  def validate_token(token): ...",
            "  → intent search, fallback keyword if fastembed missing",
        ],
        None,
    ),
    (
        "peek graph --format svg",
        "$ peek graph --format svg -o graph.svg",
        [
            "  digraph G { rankdir=LR; // polyglot: py + JS/TS",
            "    \"peek/peek/cli.py\" -> \"peek/peek/analyzer.py\";",
            "    \"peek/peek/app.js\" -> \"peek/peek/utils.js\";",
            "    \"src/main.ts\" -> \"src/lib/index.ts\";",
            "  }",
            "  → DOT/SVG/HTML via peek/graph.py (dot -Tsvg if installed)",
            "  polyglot edges: Python AST + JS/TS import/require regex",
            "  $ peek graph --format dot   # or html",
        ],
        None,
    ),
    (
        'peek --pack --ask "auth token" --semantic',
        '$ peek --pack --ask "auth token" --semantic --format md | head -20',
        [
            "  ── pack v3: 4 files • ~2.1k tokens • query=auth token (semantic) ──",
            ...
            "  ── --clip → pyperclip, --dry-run table, --diff HEAD ──",
            "  ── --staged, URL https://...tar.gz, tiktoken ──",
        ],
        None,
    ),
    (
    ...
    (
        ...
        "$ peek mcp",
        [
            "  MCP server (stdio) for Claude Code • no hard dep",
            "  Tools: peek_scan, peek_analyze, peek_find,",
            "         peek_graph, peek_pack",
            "  $ cat > ~/.config/claude/config.json",
            "    {\"mcpServers\": {\"peek\": {\"command\": \"peek\",",
            "      \"args\": [\"mcp\"]}}}",
            "  → Claude: peek analyze . → peek find \"auth token\"",
            "  → peek graph --format svg in one session",
        ],
        None,
    ),
    (
